    ...
    <div className="flex flex-row items-end gap-2">
        <FormField control = {control} name ={`menuItems.${index}.name`} render={({field})=>(
            <FormItem>
                ...
                <FormControl>
                    <Input {...field} className="bg-white" placeholder="Adobong Manok" />
                </FormControl>
            </FormItem>
        )}/>
         <FormField control = {control} name ={`menuItems.${index}.price`} render={({field})=>(
            <FormItem>
                <FormLabel className="flex items-center gap-1">Price (Php)<FormMessage/>
                </FormLabel>
                <FormControl>
                    <Input {...field} className="bg-white" placeholder="60.00" />
                </FormControl>
            </FormItem>
        )}/>
